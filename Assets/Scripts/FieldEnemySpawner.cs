		...
		if (spawning) {
            ...
            if (batchCounter >= delayBeforeBatch) { //if the time has come to spawn a batch
                for (int i = 0; i < numberInBatch; i++) { //then for the amount of enemies in a batch
                    ...
                    currentEnemy.transform.Translate(Vector3.Normalize(new Vector3(Random.Range(-1, 1), 0, Random.Range(-1, 1))) * distanceFromSpawner); //and move them away in a random direction
                }
